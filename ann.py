...
'''
# Feature Scaling not needed since already scaled
...
'''
    units: inputNodes (55) + outputNodes (1)) / 2 = 28
...
'''
    units (output_dim): output layer, only one node because dependent variable is binary,
        if (3 categories for the dependent variable) then (units = 3)
...
'''
    ...
    optimizer: the algorithm used to find optimal set of weights in the NN since
        weights are only initialized, find weights that will make the NN the most powerful
    loss: loss function within the stochastic gradient descent algorithm (the adam algorithm)
    ...
    metrics: criterion used to evaluate module (accuracy) used to improve model's performance
...
'''
    ...
    epoch: when the whole training set has been passed through the ANN
